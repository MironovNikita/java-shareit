...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto create(long userId, ItemDto itemDto) {
        ...
        if (itemDto.getRequestId() != null) {
            requestRepository.findById(itemDto.getRequestId()).ifPresentOrElse(item::setRequest, () -> {
                log.error("Server: Запрос с ID {} не найден!", itemDto.getRequestId());
    ...
    public ItemDto update(long itemId, long userId, ItemDto itemDto) {
        ...
        Item item = itemRepository.findById(itemId).orElseThrow(() -> {
            log.error("Server: Предмет с ID {} не найден!", itemId);
            return new ObjectNotFoundException("Предмет", itemId);
        });

        if (!user.equals(item.getOwner())) {
            log.error("Server: Попытка обновления предмета пользователем с ID {} при собственнике с ID {}", userId,
                    item.getOwner().getId());
    ...
    public CommentDto comment(long userId, long itemId, CommentDto commentDto) {
        ...
        if (userBookingList.isEmpty()) {
            log.error("Server: Пользователь не может оставить комментарий, т.к. не бронировал данную вещь!");
    ...
    public void delete(long id) {
        Item item = itemRepository.findById(id).orElseThrow(() -> {
            log.error("Server: Предмет с ID {} не найден!", id);
